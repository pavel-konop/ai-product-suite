    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/50 rounded-xl p-6 border border-slate-800">
        ...
        <textarea
          ...
          className="w-full h-40 bg-slate-950 border border-slate-800 rounded-lg p-4 text-sm focus:ring-2 focus:ring-blue-500 resize-none read-only:bg-slate-900/30"
        />
        ...
        {!isViewingHistory && (
          <div className="flex justify-between items-center mt-4">
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 disabled:cursor-not-allowed px-6 py-2 rounded-lg font-medium flex items-center gap-2 transition-colors"
            >
              {loading ? (
                <>
        ...
        <div className="bg-slate-900/50 rounded-xl p-6 border border-slate-800 space-y-6">
          <div className="flex justify-between items-start gap-4">
            <div className="flex-1 min-w-0">
              <h2 className="text-xl font-semibold text-blue-400">{result.projectName}</h2>
              <p className="text-slate-400 text-sm mt-1">{result.summary}</p>
            </div>
            <button
              onClick={generateLanding}
              className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm font-medium transition-colors whitespace-nowrap shrink-0"
            >
              Generate Landing
              <ArrowRight size={16} />
            </button>
          ...
          <section>
            ...
            <div className="grid gap-3">
              {result.userStories?.map((story: any, i: number) => (
                <div key={i} className="bg-slate-950/50 p-3 rounded-lg border-l-4 border-purple-500">
                  <p className="text-sm">
                    As a <span className="text-purple-400">{story.role}</span>, I want{' '}
                    <span className="text-blue-400">{story.action}</span> so that{' '}
                    <span className="text-green-400">{story.benefit}</span>
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-slate-950/50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-slate-400 mb-2">Complexity</h3>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                result.estimatedComplexity === 'Low' ? 'bg-green-900/50 text-green-400' :
                result.estimatedComplexity === 'Medium' ? 'bg-yellow-900/50 text-yellow-400' :
            ...
            <div className="bg-slate-950/50 p-4 rounded-lg">
              <h3 className="text-sm font-medium text-slate-400 mb-2">MVP Scope</h3>
              <p className="text-sm text-slate-300">{result.mvpScope}</p>
            </div>
          </div>
